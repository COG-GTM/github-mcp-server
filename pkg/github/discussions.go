...
func mapDiscussionNodesToIssues(nodes []discussionNode) []*github.Issue {
	...
	for _, n := range nodes {
		discussions = append(discussions, &github.Issue{
			...
			Labels: []*github.Label{
				{
					Name: github.Ptr(fmt.Sprintf(categoryFmt, string(n.Category.Name))),
				},
			},
		})
	}
...
func validatePaginationParams(p paginationParams) error {
	...
	return mcp.NewTool("list_discussions",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if category != "" {
				...
				discussions, err = queryDiscussionsWithCategory(ctx, client, owner, repo, &id)
			} else {
				discussions, err = queryDiscussionsAll(ctx, client, owner, repo)
			}
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
	...
	return mcp.NewTool("get_discussion",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			discussion := &github.Issue{
				...
				Labels: []*github.Label{
					{
						Name: github.Ptr(fmt.Sprintf(categoryFmt, string(d.Category.Name))),
					},
				},
			}
...
func ListDiscussionCategories(getGQLClient GetGQLClientFn, t translations.TranslationHelperFunc) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("list_discussion_categories",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if err := validatePaginationParams(paginationParams{
				First:  params.First,
				Last:   params.Last,
				After:  params.After,
