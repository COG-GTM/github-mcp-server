...
const (
	// discussionDescRepoOwner is the description for the repository owner parameter.
	discussionDescRepoOwner = "Repository owner"
	// discussionDescRepoName is the description for the repository name parameter.
	discussionDescRepoName = "Repository name"
	// discussionErrGQLClient is the error message format for GQL client failures.
	discussionErrGQLClient = "failed to get GitHub GQL client: %v"
	// discussionCategoryFmt is the format string for category labels.
	discussionCategoryFmt = "category:%s"
)

// discussionNode represents a single discussion from the GraphQL API response.
...
func mapDiscussionNodesToIssues(nodes []discussionNode) []*github.Issue {
	...
	for _, n := range nodes {
		issues = append(issues, &github.Issue{
			...
			Labels: []*github.Label{
				{
					Name: github.Ptr(fmt.Sprintf(discussionCategoryFmt, string(n.Category.Name))),
				},
			},
		})
	}
...
// queryDiscussionsWithCategory fetches discussions filtered by category.
...
// queryDiscussionsAll fetches all discussions without category filtering.
...
func ListDiscussions(getGQLClient GetGQLClientFn, t translations.TranslationHelperFunc) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("list_discussions",
			...
			mcp.WithString("owner",
				mcp.Required(),
				mcp.Description(discussionDescRepoOwner),
			),
			mcp.WithString("repo",
				mcp.Required(),
				mcp.Description(discussionDescRepoName),
			),
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if err != nil {
				return mcp.NewToolResultError(fmt.Sprintf(discussionErrGQLClient, err)), nil
			}
			...
			out, err := json.Marshal(mapDiscussionNodesToIssues(nodes))
			if err != nil {
	...
	return mcp.NewTool("get_discussion",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			discussion := &github.Issue{
				...
				Labels: []*github.Label{
					{
						Name: github.Ptr(fmt.Sprintf(discussionCategoryFmt, string(d.Category.Name))),
					},
				},
			}
...
func GetDiscussionComments(getGQLClient GetGQLClientFn, t translations.TranslationHelperFunc) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("get_discussion_comments",
			...
			mcp.WithString("owner", mcp.Required(), mcp.Description(discussionDescRepoOwner)),
			mcp.WithString("repo", mcp.Required(), mcp.Description(discussionDescRepoName)),
