...
func buildSearchTool(toolName, description, userTitle, queryDesc string, t translations.TranslationHelperFunc) mcp.Tool {
	return mcp.NewTool(toolName,
		mcp.WithDescription(t(description+"_DESCRIPTION", description)),
		mcp.WithToolAnnotation(mcp.ToolAnnotation{
			Title:        t(userTitle+"_USER_TITLE", userTitle),
...
func buildSearchHandler(searchType string, getClient GetClientFn, searchFunc func(context.Context, *github.Client, string, *github.SearchOptions) (interface{}, *github.Response, error)) server.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		result, resp, err := searchFunc(ctx, client, query, opts)
		if errorResult, err := handleSearchResponse(ctx, resp, err, fmt.Sprintf("failed to search %s with query '%s'", searchType, query)); errorResult != nil || err != nil {
...
func SearchRepositories(getClient GetClientFn, t translations.TranslationHelperFunc) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	tool = mcp.NewTool("search_repositories",
	...
	handler = buildSearchHandler("repositories", getClient, func(ctx context.Context, client *github.Client, query string, opts *github.SearchOptions) (interface{}, *github.Response, error) {
...
func userOrOrgHandler(accountType string, getClient GetClientFn) server.ToolHandlerFunc {
	return buildSearchHandler(accountType+"s", getClient, func(ctx context.Context, client *github.Client, query string, opts *github.SearchOptions) (interface{}, *github.Response, error) {
...
func SearchUsers(getClient GetClientFn, t translations.TranslationHelperFunc) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return buildSearchTool("search_users", "Search for GitHub users exclusively", "Search users", "Search query using GitHub users search syntax scoped to type:user", t),
...
func SearchOrgs(getClient GetClientFn, t translations.TranslationHelperFunc) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return buildSearchTool("search_orgs", "Search for GitHub organizations exclusively", "Search organizations", "Search query using GitHub organizations search syntax scoped to type:org", t),
