...
func main() {
	...
	if err == nil && serverCmd != "" {
		loadSchemaAndAddCommands(serverCmd, toolsCmd, prettyPrint)
	}
...
// loadSchemaAndAddCommands loads the schema from the server and adds tool commands
func loadSchemaAndAddCommands(serverCmd string, toolsCmd *cobra.Command, prettyPrint bool) {
	jsonRequest, err := buildJSONRPCRequest("tools/list", "", nil)
	if err == nil {
		response, err := executeServerCommand(serverCmd, jsonRequest)
		if err == nil {
