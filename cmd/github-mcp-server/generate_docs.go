...
// extractPropertyType determines the type string for a schema property map.
...
// extractPropertyDescription returns the description string from a schema property map.
...
func formatParamLine(propName string, prop interface{}, required bool) string {
...
func generateToolDoc(tool mcp.Tool) string {
	...
	for _, propName := range paramNames {
		...
		lines = append(lines, formatParamLine(propName, prop, required))
	}
