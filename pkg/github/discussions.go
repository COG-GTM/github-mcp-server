...
const (
	...
	errFailedGQLClient  = "failed to get GitHub GQL client: %v"
)
...
func executeDiscussionsQueryWithCategory(ctx context.Context, client *githubv4.Client, owner, repo string, categoryID *githubv4.ID) ([]discussionNode, error) {
	var query struct {
		Repository struct {
			Discussions struct {
...
func executeDiscussionsQueryWithoutCategory(ctx context.Context, client *githubv4.Client, owner, repo string) ([]discussionNode, error) {
	var query struct {
		Repository struct {
			Discussions struct {
				Nodes []discussionNode
...
func ListDiscussions(getGQLClient GetGQLClientFn, t translations.TranslationHelperFunc) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("list_discussions",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if err != nil {
				return mcp.NewToolResultError(fmt.Sprintf(errFailedGQLClient, err)), nil
			}
			...
			if categoryID != nil {
				nodes, err = executeDiscussionsQueryWithCategory(ctx, client, owner, repo, categoryID)
			} else {
				nodes, err = executeDiscussionsQueryWithoutCategory(ctx, client, owner, repo)
			}
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
