...
// discussionNode represents a GraphQL discussion node structure
...
func convertDiscussionNodeToIssue(n discussionNode) *github.Issue {
	return &github.Issue{
		...
		Labels: []*github.Label{
			{
				Name: github.Ptr(fmt.Sprintf(formatCategoryLabel, string(n.Category.Name))),
			},
		},
	}
}

func queryDiscussions(ctx context.Context, client *githubv4.Client, owner, repo string, categoryID *githubv4.ID) ([]discussionNode, error) {
	if categoryID != nil {
		var query struct {
			Repository struct {
				Discussions struct {
...
func ListDiscussions(getGQLClient GetGQLClientFn, t translations.TranslationHelperFunc) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("list_discussions",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			nodes, err := queryDiscussions(ctx, client, owner, repo, categoryID)
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
			...
			for _, n := range nodes {
				discussions = append(discussions, convertDiscussionNodeToIssue(n))
			}
	...
	return mcp.NewTool("get_discussion",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			discussion := &github.Issue{
				...
				Labels: []*github.Label{
					{
						Name: github.Ptr(fmt.Sprintf(formatCategoryLabel, string(d.Category.Name))),
					},
				},
			}
