...
func TestSnapshotDoesNotExistNotInCI(t *testing.T) {
	...
	require.NoError(t, err)
	path := filepath.Join("__toolsnaps__", DummySnapFile)
...
func TestSnapshotExistsMatch(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join("__toolsnaps__", DummySnapFile), b, 0600))
...
func TestSnapshotExistsDiff(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll("__toolsnaps__", 0700))
	require.NoError(t, os.WriteFile(filepath.Join("__toolsnaps__", DummySnapFile), []byte(`{"name":"foo","value":1}`), 0600))
...
func TestMalformedSnapshotJSON(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll("__toolsnaps__", 0700))
	require.NoError(t, os.WriteFile(filepath.Join("__toolsnaps__", DummySnapFile), []byte(`not-json`), 0600))
	tool := dummyTool{"foo", 42}
