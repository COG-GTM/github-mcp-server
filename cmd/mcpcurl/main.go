...
const stdioServerCmdFlag = "stdio-server-cmd"
...
func main() {
	...
	if err == nil && serverCmd != "" {
		setupDynamicCommands(serverCmd, prettyPrint)
	}
...
// setupDynamicCommands fetches schema from server and adds dynamic tool commands
func setupDynamicCommands(serverCmd string, prettyPrint bool) {
	jsonRequest, err := buildJSONRPCRequest("tools/list", "", nil)
	if err == nil {
		response, err := executeServerCommand(serverCmd, jsonRequest)
		if err == nil {
