...
func queryDiscussionsWithoutCategory(ctx context.Context, client *githubv4.Client, owner, repo string) ([]discussionNode, error) {
	var query struct {
		Repository struct {
			Discussions struct {
				Nodes []discussionNode
	...
	return mcp.NewTool("list_discussions",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if category != "" {
				...
				nodes, err = queryDiscussionsWithoutCategory(ctx, client, owner, repo)
			}
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
