...
const (
	// Common parameter descriptions used across discussion tools.
	descRepoOwner = "Repository owner"
	descRepoName  = "Repository name"

	// Common error format for GQL client failures.
	errGQLClientFmt = "failed to get GitHub GQL client: %v"

	// Format string for category labels.
	categoryLabelFmt = "category:%s"
)

// discussionNode holds the common fields returned by the GitHub GraphQL
// discussions query.  Both the filtered and unfiltered query shapes share
...
// discussionNodeToIssue converts a discussionNode into a *github.Issue.
func discussionNodeToIssue(n discussionNode) *github.Issue {
	return &github.Issue{
...
func ListDiscussions(getGQLClient GetGQLClientFn, t translations.TranslationHelperFunc) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("list_discussions",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			nodes, err := queryDiscussions(ctx, client, owner, repo, category)
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
			...
			for _, n := range nodes {
				discussions = append(discussions, discussionNodeToIssue(n))
			}
...
// queryDiscussions executes the GraphQL discussions query, optionally
// filtering by category.  It returns the raw discussion nodes.
func queryDiscussions(ctx context.Context, client *githubv4.Client, owner, repo, category string) ([]discussionNode, error) {
	if category != "" {
