...
// discussionNode represents a single discussion node returned by the GitHub GraphQL API.
...
// discussionNodesToIssues converts GraphQL discussion nodes to GitHub Issue representations.
func discussionNodesToIssues(nodes []discussionNode) []*github.Issue {
...
// fetchDiscussions queries discussions from the GitHub GraphQL API, optionally filtered by category.
func fetchDiscussions(ctx context.Context, client *githubv4.Client, owner, repo string, categoryID *githubv4.ID) ([]*github.Issue, error) {
	if categoryID != nil {
		var query struct {
			Repository struct {
				Discussions struct {
		...
		return discussionNodesToIssues(query.Repository.Discussions.Nodes), nil
	}
...
func ListDiscussions(getGQLClient GetGQLClientFn, t translations.TranslationHelperFunc) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("list_discussions",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			discussions, err := fetchDiscussions(ctx, client, owner, repo, categoryID)
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
